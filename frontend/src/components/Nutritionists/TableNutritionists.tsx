...
import { loadColumns } from './configureNutritionistsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListNutritionists from './ListNutritionists';

const perPage = 10;

const TableSampleNutritionists = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: nutritionistsNotify,
    refetch,
  } = useAppSelector((state) => state.nutritionists);
  ...
  useEffect(() => {
    if (nutritionistsNotify.showNotification) {
      notify(
        nutritionistsNotify.typeNotification,
        nutritionistsNotify.textNotification,
      );
    }
  }, [nutritionistsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/nutritionists/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/nutritionists/nutritionists-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {nutritionists && Array.isArray(nutritionists) && !showGrid && (
        <ListNutritionists
...
export default TableSampleNutritionists;
