...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/ebooks/ebooksSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureEbooksCols';
...
const TableSampleEbooks = ({
  ...
  const {
    ...
    notify: ebooksNotify,
    refetch,
  } = useAppSelector((state) => state.ebooks);
  ...
  useEffect(() => {
    if (ebooksNotify.showNotification) {
      notify(ebooksNotify.typeNotification, ebooksNotify.textNotification);
    }
  }, [ebooksNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/ebooks/ebooks-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {ebooks && Array.isArray(ebooks) && !showGrid && (
        <ListEbooks
          ebooks={ebooks}
          loading={loading}
...
export default TableSampleEbooks;
