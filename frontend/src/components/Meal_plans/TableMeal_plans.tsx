...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/meal_plans/meal_plansSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureMeal_plansCols';
...
const TableSampleMeal_plans = ({
  ...
  const {
    ...
    loading,
    count,
    notify: meal_plansNotify,
    refetch,
  } = useAppSelector((state) => state.meal_plans);
  ...
  useEffect(() => {
    if (meal_plansNotify.showNotification) {
      notify(
        meal_plansNotify.typeNotification,
        meal_plansNotify.textNotification,
      );
    }
  }, [meal_plansNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/meal_plans/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/meal_plans/meal_plans-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/meal_plans/meal_plans-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
...
export default TableSampleMeal_plans;
